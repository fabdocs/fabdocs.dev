  ...
  return (
    ...
    >
      <div className="px-3 py-2 flex-1">
        <p className="text-sm font-medium mb-2">Ask fabdocs AI</p>
        <p className="text-xs text-fd-muted-foreground">
          Grounded in the docs. AI can be inaccurate — verify against the linked pages.
        </p>
      </div>
        ...
        return (
          ...
          >
            ...
            {call.state === 'output-error' || call.state === 'output-denied' ? (
              ...
              <p>{!call.output ? 'Searching…' : `${(call.output as unknown[]).length} search results`}</p>
            )}
          </div>
        );
      })}
...
function isLimitError(message: string) {
  return /free AI questions|Upgrade to Pro|free limit/i.test(message);
}
    ...
    >
      {messages.length === 0 ? (
        <div className="text-sm text-fd-muted-foreground/80 size-full flex flex-col items-center justify-center text-center gap-2">
          ...
          <p onClick={(e) => e.stopPropagation()}>Ask about Fabric — start below.</p>
        </div>
      ) : (
        <div className="flex flex-col px-3 gap-4">
          {chat.error && (
            <div className="p-3 bg-fd-secondary text-fd-secondary-foreground border rounded-lg">
              {isLimitError(chat.error.message) ? (
                <>
